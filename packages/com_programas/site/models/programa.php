 ...
 * @since  1.6
 */
class ProgramasModelPrograma extends JModelItem
{
	/**
	...
	public function &getData($id = null)
	{
		...
		}if (isset($this->_item->modified_by) )
		{
			$this->_item->modified_by_name = JFactory::getUser($this->_item->modified_by)->name;
				...
				if ( isset($this->_item->genre) ) {
					// Catch the item tags (string with ',' coma glue)
					$tags = explode(",",$this->_item->genre);
					...
					if (!empty($namedTags)) {
						$this->_item->genre = implode(', ',$namedTags);
					} else {
						$this->_item->genre = !empty($this->_item->my_tags) ? ($this->_item->my_tags) : ("");
					}
		}

				// Get the title of every option selected.

				$options = json_decode($this->_item->days_of_the_week);

				$options_text = array();

				foreach($options as $option){
						$options_text[] = JText::_('COM_PROGRAMAS_PROGRAMAS_DAYS_OF_THE_WEEK_OPTION_' . $option);

				}
					$this->_item->days_of_the_week = !empty($options_text) ? implode(',', $options_text) : $this->_item->days_of_the_week;
