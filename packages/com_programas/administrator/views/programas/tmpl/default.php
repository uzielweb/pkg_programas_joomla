...
$document->addStyleSheet(JUri::root() . 'administrator/components/com_programas/assets/css/programas.css');
$document->addStyleSheet(JUri::root() . 'media/com_programas/css/list.css');

$user      = JFactory::getUser();
$userId    = $user->get('id');
$listOrder = $this->state->get('list.ordering');
$listDirn  = $this->state->get('list.direction');
$canOrder  = $user->authorise('core.edit.state', 'com_programas');
$saveOrder = $listOrder == 'a.`ordering`';

if ($saveOrder)
{
	$saveOrderingUrl = 'index.php?option=com_programas&task=programas.saveOrderAjax&tmpl=component';
	JHtml::_('sortablelist.sortable', 'programaList', 'adminForm', strtolower($listDirn), $saveOrderingUrl);
}
		...
		<div id="j-main-container">
			...
			<table class="table table-striped" id="programaList">
				...
				<?php foreach ($this->items as $i => $item) :
					...
					$canCreate  = $user->authorise('core.create', 'com_programas');
					...
					<tr class="row<?php echo $i % 2; ?>">
						...
						<?php if (isset($this->items[0]->state)): ?>
							<td class="center">
								<?php echo JHtml::_('jgrid.published', $item->state, $i, 'programas.', $canChange, 'cb'); ?>
</td>
						<?php endif; ?>
				...
				<?php endif; ?>
				<?php if ($canEdit) : ?>
					<a href="<?php echo JRoute::_('index.php?option=com_programas&task=programa.edit&id='.(int) $item->id); ?>">
