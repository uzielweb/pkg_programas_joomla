 ...
 * @since  1.6
 */
class ProgramasModelPrograma extends JModelAdmin
{
	/**
	 * @var      string    The prefix to use with controller messages.
	 * @since    1.6
	 */
	protected $text_prefix = 'COM_PROGRAMAS';

	/**
	 * @var   	string  	Alias to manage history control
	 * @since   3.2
	 */
	public $typeAlias = 'com_programas.programa';
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm(
			'com_programas.programa', 'programa',
			array('control' => 'jform',
				'load_data' => $loadData
			)
		);
			...
			if($form->getFieldAttribute('start_time', 'default') == 'NOW'){
				$form->setFieldAttribute('start_time', 'default', date('H:i:s'));
			}
			if($form->getFieldAttribute('end_time', 'default') == 'NOW'){
				$form->setFieldAttribute('end_time', 'default', date('H:i:s'));
			}
		if (empty($form))
		{
			return false;
	...
	/**
	 * Method to duplicate an Programa
	 *
	...
	public function duplicate(&$pks)
	{
		...
		// Access checks.
		if (!$user->authorise('core.create', 'com_programas'))
		{
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id))
		{
			...
			{
				$db = JFactory::getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__programas');
				$max             = $db->loadResult();
