...
<?php
        if (($gmday == $theday) and ($gmdate >= $theinitialtime) and ($gmdate <= $thefinaltime) and ($item->state == '1')): ?>
    <?php
            if ($item->category == $module_category): ?>

        <?php
                $haveprogram = '1'; ?>
        <?php
                $popup_title = $category_title; ?>

         <?php
                if (in_array('program_name', $enabled) or $params->get('enable_defaults')): ?>
        <h4 class="program_name"><?php
                    echo $item->program_name; ?></h4>
         <?php
                endif; ?>
        <?php
                if (in_array('category_title', $enabled) or $params->get('enable_defaults')): ?>
        <h5> <?php
                    echo $category_title ?> </h5>
        <?php
                endif; ?>
        <?php
        ...
        <?php
                if (in_array('program_link', $enabled) or $params->get('enable_defaults')): ?>
        <p class="program_link"><a href="<?php
                    echo $item->program_link; ?>" target="_blank"><?php
                    echo $item->program_link; ?></a></p>
        <?php
                endif; ?>
        <?php
                if ($params->get('custom_message')): ?>
        <div class="span12 col-md-12 custom_message"><?php
                    echo $params->get('custom_message'); ?></div>
        <?php
                endif; ?>
        <?php
                if (in_array('program_description', $enabled)): ?>
        <div class="span12 col-md-12 program_description"><?php
                    echo $item->program_description; ?></div>
        <?php
                endif; ?>
        <?php
                if (in_array('broadcaster_image', $enabled) or $params->get('enable_defaults')): ?>
        <p class="broadcaster_image"><a href="<?php
                    echo $item->broadcaster_image; ?>" class="modal"><img src="<?php
                    echo $item->broadcaster_image; ?>" alt="<?php
                    echo $item->broadcaster_name; ?>" /></a></p>
        <?php
                endif; ?>
        <?php
                if (in_array('broadcaster_name', $enabled) or $params->get('enable_defaults')): ?>
        <h5 class="broadcaster_name"><?php
                    echo $item->broadcaster_name; ?></h5>
        <?php
                endif; ?>
        <div class="broadcaster_social col-md-12">
         <?php
                if (in_array('broadcaster_whatsapp', $enabled) or $params->get('enable_defaults')): ?>
        <p><i class="fa fa-whatsapp"></i> <?php
                    echo $item->broadcaster_whatsapp; ?> </p>
        <?php
                endif; ?>
        <?php
                if (in_array('broadcaster_facebook', $enabled)): ?>
        <a href="<?php
                    echo $item->broadcaster_facebook; ?>" target="_blank"><i class="fa fa-facebook"></i></a>
        <?php
                endif; ?>
        <?php
                if (in_array('broadcaster_twitter', $enabled)): ?>
        <a href="<?php
                    echo $item->broadcaster_twitter; ?>" target="_blank"><i class="fa fa-twitter"></i></a>
        <?php
                endif; ?>
        <?php
                if (in_array('broadcaster_instagram', $enabled)): ?>
        <a href="<?php
                    echo $item->broadcaster_instagram; ?>" target="_blank"><i class="fa fa-instagram"></i></a>
        <?php
                endif; ?>
        <?php
                if (in_array('broadcaster_snapchat', $enabled)): ?>
        <a href="<?php
                    echo $item->broadcaster_snapchat; ?>" target="_blank"><i class="fa fa-snapchat"></i></a>
        <?php
                endif; ?>
       <?php
                if (in_array('broadcaster_telegram', $enabled)): ?>
        <a href="<?php
                    echo $item->broadcaster_telegram; ?>" target="_blank"><i class="fa fa-paper-plane"></i></a>
        <?php
                endif; ?>

         <?php
                if (in_array('broadcaster_email', $enabled)): ?>
        <a href="mailto:<?php
                    echo $item->broadcaster_email; ?>" target="_blank"><i class="fa fa-envelope"></i></a>
        <?php
                endif; ?>
         <?php
                if (in_array('broadcaster_link', $enabled)): ?>
        <a href="<?php
                    echo $item->broadcaster_link; ?>" target="_blank"><i class="fa fa-globe"></i></a>
        <?php
                endif; ?>
        <?php
                if (in_array('broadcaster_bio', $enabled)): ?>
        <div class="span12 col-md-12 broadcaster_bio"><?php
                    echo $item->broadcaster_bio; ?></div>
        <?php
                endif; ?>
        </div>
        <?php
        ...
        <?php
         ...
         <?php
                if ($params->get('module_or_custom_script') == '1'): ?>
        <div class="span12 col-md-12">
            <?php
                    echo $params->get('custom_script'); ?>
        </div>
        <?php
                endif; ?>
    <?php
...
if ($haveprogram == '0'): ?>
        <h4 class="program_name"><?php
    echo JText::_('MOD_PROGRAMAS_OFFLINE'); ?></h4>
        <?php
    if (in_array('category_title', $enabled)): ?>
        <h5> <?php
        echo $category_title ?> </h5>
        <?php
    endif; ?>
...
jQuery("#btnPrint<?php
    ...
    echo $params->get('popup_height'); ?>,50%,50%,directories=0,titlebar=0,toolbar=0,location=0,status=0,menubar=0,scrollbars=no,resizable=no");
            printWindow.document.write('<html><head><title>' + '<?php
    echo $popup_title; ?>' + '</title>');
            ...
            printWindow.document.write('<?php
    echo $params->get('builtin_script'); ?>');
